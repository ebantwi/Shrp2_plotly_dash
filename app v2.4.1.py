...
hrnew = pd.read_csv('Data/Speed_1819.csv')
ref = pd.read_csv('Data/refspeed.csv')
reliability = pd.read_csv('data/For_Reliability.csv')
hrnew = hrnew.merge(ref, on='LinkDir', how='left')
# hr15 = pd.read_csv('data/speeds_15_17.csv')
lottr = pd.read_csv('data/lottr_mod.csv', usecols=['LinkDir', 'LOTTR', 'Year'])
measures =  pd.read_csv('data/Measures_with_outliers.csv')
measuresw = pd.read_csv('data/Measures_without_outliers.csv')
...
def showHS(tab, r, b, e, d, c):
    if r is not None:
        if tab == 'tab-1':
            df = routequery1819(r, b, e, district=d, county=c).sort_values('BEGIN_POINT')
            df = df.reset_index(drop=True)
            df.rename(columns={'SECTION_LENGTH': 'Length'}, inplace=True)
            df.loc[df.loc[df['AllRds_Dir'] == 'Cardinal', 'BEGIN_POINT'].idxmin(), 'BEGIN_POINT'] = b
            df.loc[df.loc[df['AllRds_Dir'] != 'Cardinal', 'BEGIN_POINT'].idxmin(), 'BEGIN_POINT'] = b
            df.loc[df.loc[df['AllRds_Dir'] == 'Cardinal', 'END_POINT'].idxmax(), 'END_POINT'] = e
            df.loc[df.loc[df['AllRds_Dir'] != 'Cardinal', 'END_POINT'].idxmax(), 'END_POINT'] = e

            df['Length'] = abs(df['END_POINT'] - df['BEGIN_POINT'])

            lc, lnc = abs(e-b), abs(e-b)

            dfspd = df.merge(hrnew, how='left', on='LinkDir')
            dfspd['refspeed'] = np.where(dfspd['F_SYSTEM'] == 1, dfspd['Pcnt85Yr'], dfspd['WeightedAvgDt'])
            dfspd['RefTT_dt'] = dfspd['Length'] / dfspd['refspeed']
            dfspd['MeanTT_dt'] = dfspd['Length'] / dfspd['AvgSpeed']

            cc = dfspd[dfspd.AllRds_Dir == 'Cardinal'].sort_values('BEGIN_POINT').reset_index(drop=True)
            ncc = dfspd[dfspd.AllRds_Dir == 'NonCardinal'].sort_values('BEGIN_POINT').reset_index(drop=True)

            cc.dropna(subset=['AvgSpeed'], inplace=True)
            ncc.dropna(subset=['AvgSpeed'], inplace=True)

            cref = cc.drop_duplicates(['ROUTE_ID', 'BEGIN_POINT', 'END_POINT']).RefTT_dt.sum()
            ncref = ncc.drop_duplicates(['ROUTE_ID', 'BEGIN_POINT', 'END_POINT']).RefTT_dt.sum()

            creflength = cc.drop_duplicates(['ROUTE_ID', 'BEGIN_POINT', 'END_POINT']).Length.sum()
            ncreflength = ncc.drop_duplicates(['ROUTE_ID', 'BEGIN_POINT', 'END_POINT']).Length.sum()

            AADTc = np.average(df.loc[df.AllRds_Dir == 'Cardinal', 'AADT'],
                               weights=df.loc[df.AllRds_Dir == 'Cardinal', 'Length'])

            AADTnc = np.average(df.loc[df.AllRds_Dir == 'NonCardinal', 'AADT'],
                                weights=df.loc[df.AllRds_Dir == 'NonCardinal', 'Length'])

            speedlimitc = np.average(df.loc[df.AllRds_Dir == 'Cardinal', 'SPEED_LIMIT_LWA'],
                                     weights=df.loc[df.AllRds_Dir == 'Cardinal', 'Length'])

            speedlimitnc = np.average(df.loc[df.AllRds_Dir != 'Cardinal', 'SPEED_LIMIT_LWA'],
                                      weights=df.loc[df.AllRds_Dir != 'Cardinal', 'Length'])

            ## -----------------------------aggregate link values to route level values -----------------------------
            lwacc = lambda x: np.average(x[~np.isnan(x)], weights=c.loc[x[~np.isnan(x)].index, 'Length'])
            lwancc = lambda x: np.average(x[~np.isnan(x)], weights=nc.loc[x[~np.isnan(x)].index, 'Length'])

            cc = cc.groupby(['Hour', 'Year'], as_index=False).agg(
                {'MeanTT_dt': 'sum', 'Length': 'sum'})

            ncc = ncc.groupby(['Hour', 'Year'], as_index=False).agg(
                {'MeanTT_dt': 'sum', 'Length': 'sum'})

            cc['RouteMeanSpeed_dt'] = cc['Length'] / cc['MeanTT_dt']
            ncc['RouteMeanSpeed_dt'] = ncc['Length'] / ncc['MeanTT_dt']

            cc['RouteRefSpeed'] = creflength / cref  # space mean reference speed based on links with data
            cc['RouteRefSpeed'] = np.where(cc['RouteRefSpeed'] > speedlimitc, speedlimitc, cc['RouteRefSpeed'])

            ncc['RouteRefSpeed'] = ncreflength / ncref
            ncc['RouteRefSpeed'] = np.where(ncc['RouteRefSpeed'] > speedlimitnc, speedlimitnc, ncc['RouteRefSpeed'])

            cc['perc'] = cc['Hour'].map(lambda x: hfs[x])
            ncc['perc'] = ncc['Hour'].map(lambda x: hfs[x])

            cc['hrvol'] = np.ceil(cc['perc'] * (AADTc / 2))
            ncc['hrvol'] = np.ceil(ncc['perc'] * (AADTnc / 2))

            cc['Hour_VMT'] = cc['hrvol'] * lc
            cc['Hourly_VHT'] = cc['Hour_VMT'] / cc['RouteMeanSpeed_dt']
            cc['delay'] = (cc['Hourly_VHT'] - (cc['Hour_VMT'] / cc['RouteRefSpeed'])) * 260
            cc['delay'] = np.where(cc['delay'] < 0, 0, cc['delay'])

            ncc['Hour_VMT'] = ncc['hrvol'] * lnc
            ncc['Hourly_VHT'] = ncc['Hour_VMT'] / ncc['RouteMeanSpeed_dt']
            ncc['delay'] = (ncc['Hourly_VHT'] - (ncc['Hour_VMT'] / ncc['RouteRefSpeed'])) * 260
            ncc['delay'] = np.where(ncc['delay'] < 0, 0, ncc['delay'])

            dfperc = df.merge(reliability, how='left', on='LinkDir')
            dfperc['MeanTT_dt'] = dfperc['Length'] / dfperc['WeightedAvgSpeedHour']
            dfperc['Pcnt95TT_dt'] = dfperc['Length'] / dfperc['Pcnt5SpeedHour']  # link All-Vehicle Travel times
            dfperc['Pcnt80TT_dt'] = dfperc['Length'] / dfperc['Pcnt20SpeedHour']
            dfperc['Pcnt50TT_dt'] = dfperc['Length'] / dfperc['Pcnt50SpeedHour']

            cp = dfperc[dfperc.AllRds_Dir == 'Cardinal'].sort_values('BEGIN_POINT').reset_index(drop=True)
            ncp = dfperc[dfperc.AllRds_Dir == 'NonCardinal'].sort_values('BEGIN_POINT').reset_index(drop=True)
            cp.dropna(subset=['WeightedAvgSpeedHour'], inplace=True)
            ncp.dropna(subset=['WeightedAvgSpeedHour'], inplace=True)

            cp = cp.groupby('Hour', as_index=False).agg(
                {'MeanTT_dt': 'sum', 'Pcnt95TT_dt': 'sum', 'Pcnt80TT_dt': 'sum', 'Pcnt50TT_dt': 'sum',
                 'Length': 'sum'})
            ncp = ncp.groupby('Hour', as_index=False).agg(
                {'MeanTT_dt': 'sum', 'Pcnt95TT_dt': 'sum', 'Pcnt80TT_dt': 'sum', 'Pcnt50TT_dt': 'sum',
                 'Length': 'sum'})

            cp['RouteMeanSpeed_dt'] = cp['Length'] / cp['MeanTT_dt']
            cp['RoutePcnt5Speed_dt'] = cp['Length'] / cp['Pcnt95TT_dt']
            cp['RoutePcnt20Speed_dt'] = cp['Length'] / cp['Pcnt80TT_dt']
            cp['RoutePcnt50Speed_dt'] = cp['Length'] / cp['Pcnt50TT_dt']

            ncp['RouteMeanSpeed_dt'] = ncp['Length'] / ncp['MeanTT_dt']
            ncp['RoutePcnt5Speed_dt'] = ncp['Length'] / ncp['Pcnt95TT_dt']
            ncp['RoutePcnt20Speed_dt'] = ncp['Length'] / ncp['Pcnt80TT_dt']
            ncp['RoutePcnt50Speed_dt'] = ncp['Length'] / ncp['Pcnt50TT_dt']

            cp['RouteRefSpeedUncapped'] = creflength / cref  # space mean reference speed based on links with data
            cp['RouteRefSpeed'] = np.where(cp['RouteRefSpeedUncapped'] > speedlimitc, speedlimitc,
                                           cp['RouteRefSpeedUncapped'])

            ncp['RouteRefSpeedUncapped'] = creflength / cref  # space mean reference speed based on links with data
            ncp['RouteRefSpeed'] = np.where(ncp['RouteRefSpeedUncapped'] > speedlimitc, speedlimitc,
                                            ncp['RouteRefSpeedUncapped'])
...
######################################################################
            mclwa = lambda x: round(np.average(x[~np.isnan(x)], weights=mc.loc[x[~np.isnan(x)].index, 'Length']), 2)

            mcwlwa = lambda x: round(np.average(x[~np.isnan(x)], weights=mcw.loc[x[~np.isnan(x)].index, 'Length']), 2)
########################################################################
            # CARDINAL
            dc = hr[hr['LinkDir'].isin(c)]  #
            lot = lottr[lottr['LinkDir'].isin(c)]
######################################################################
            mc = measures[measures['LinkDir'].isin(c)]
            mc = mc.iloc[:, 1:]

            mcw = measuresw[measuresw['LinkDir'].isin(c)]
            mcw = mcw.iloc[:, 1:]
######################################################################

            drt, leng, adt = diction(df['LinkDir'], df['AllRds_Dir'], df['Length'], df['AADT'])
            dc['Length'] = dc['LinkDir'].map(lambda x: leng[x])
            dc['AADT'] = dc['LinkDir'].map(lambda x: adt[x])
            lot['Length'] = lot['LinkDir'].map(lambda x: leng[x])

            mc['Length'] = mc['LinkDir'].map(lambda x: leng[x])
            mcw['Length'] = mcw['LinkDir'].map(lambda x: leng[x])
...
######################################################################
            mcagg = mc.groupby('Year', as_index=False).agg(mclwa)
            mc18, mc19 = mcagg.iloc[0], mcagg.iloc[1]

            mcaggw = mcw.groupby('Year', as_index=False).agg(mcwlwa)
            mcw18, mcw19 = mcaggw.iloc[0], mcaggw.iloc[1]
######################################################################
            rf = round(np.average(df.loc[(df.AllRds_Dir == 'Cardinal'), 'refspeed'],
                                  weights=df.loc[(df.AllRds_Dir == 'Cardinal'), 'Length']), 2)
...
######################################################################
            mcn = measures[measures['LinkDir'].isin(nc)]
            mcn = mcn.iloc[:, 1:]

            mcwn = measuresw[measuresw['LinkDir'].isin(nc)]
            mcwn = mcwn.iloc[:, 1:]
######################################################################
...
###################################################################
            mclwan = lambda x: round(np.average(x[~np.isnan(x)], weights=mcn.loc[x[~np.isnan(x)].index, 'Length']), 2)

            mcwlwan = lambda x: round(np.average(x[~np.isnan(x)], weights=mcwn.loc[x[~np.isnan(x)].index, 'Length']), 2)
###################################################################
...
################################################################
            mcn['Length'] = mcn['LinkDir'].map(lambda x: leng[x])
            mcwn['Length'] = mcwn['LinkDir'].map(lambda x: leng[x])
################################################################
...
####################################################################
            mcaggn = mcn.groupby('Year', as_index=False).agg(mclwan)
            mcn18, mcn19 = mcaggn.iloc[0], mcaggn.iloc[1]

            mcwaggn = mcwn.groupby('Year', as_index=False).agg(mcwlwan)
            mcwn18, mcwn19 = mcwaggn.iloc[0], mcwaggn.iloc[1]
#####################################################################
            rfn = round(np.average(df.loc[(df.AllRds_Dir != 'Cardinal'), 'refspeed'],
                                  weights=df.loc[(df.AllRds_Dir != 'Cardinal'), 'Length']), 2)
...
########################################################################
            ...
            max_d = dl['delT'].max() + 1000 if (dl['delT'].max() > dln['delT'].max()) else dln['delT'].max() + 1000

            # cardinal hourly speed figure
            figc = make_subplots(specs=[[{"secondary_y": True}]])

            figc.add_trace(go.Bar(name='Volume',
                                  x=cc.loc[cc.Year == 2018, "Hour"],
                                  y=cc.loc[cc.Year == 2018, "hrvol"],
                                  marker_color=bar_color,
                                  opacity=0.5, ),
                           secondary_y=False, )

            figc.add_trace(go.Scatter(x=cc.loc[cc.Year == 2018, "Hour"],
                                      y=cc.loc[cc.Year == 2018, "RouteMeanSpeed_dt"],
                                      mode='lines+markers',
                                      name='2018 Speed',
            ...
            figc.add_trace(go.Scatter(x=cc.loc[cc.Year == 2019, "Hour"],
                                      y=cc.loc[cc.Year == 2019, "RouteMeanSpeed_dt"],
                                      mode='lines+markers',
                                      name='2019 Speed',
                           ...
                           secondary_y=True, )
            figc.add_trace(go.Scatter(x=cc.loc[cc.Year == 2019, "Hour"],
                                      y=cc.loc[cc.Year == 2019, "RouteRefSpeed"],
            ...
            figc.update_yaxes(title_text="Speed(mph)", range=[5, 90], secondary_y=True) #[min_y, max_y]
            figc.update_yaxes(title_text="Volume", secondary_y=False)
            ########################################################

            figcp = make_subplots(specs=[[{"secondary_y": True}]])

            figcp.add_trace(go.Scatter(x=cp.loc[:, "Hour"],
                                       y=cp.loc[:, 'RouteMeanSpeed_dt'],
                                       mode='lines+markers',
                                       name='Mean Spd',
            ...
            figcp.add_trace(go.Scatter(x=cp.loc[:, "Hour"],
                                       y=cp.loc[:, "RoutePcnt20Speed_dt"],
                                       mode='lines+markers',
                                       name='20th Percentile spd',
                                       marker=dict(color='blue', ),
                                       line=dict(color='blue')),
                            secondary_y=True, )

            figcp.add_trace(go.Scatter(x=cp.loc[:, "Hour"],
                                       y=cp.loc[:, "RoutePcnt50Speed_dt"],
                                       mode='lines+markers',
                                       name='50th Percentile spd',
                                       marker=dict(color='green', ),
                                       line=dict(color='green')),
                            secondary_y=True, )

            figcp.add_trace(go.Scatter(x=cp.loc[:, "Hour"],
                                       y=cp.loc[:, "RoutePcnt5Speed_dt"],
                                       mode='lines+markers',
                                       name='5th Percentile spd',
                                       marker=dict(color='red', ),
                                       line=dict(color='red')),
                            secondary_y=True, )

            figcp.add_trace(go.Scatter(x=cp.loc[:, "Hour"],
                                       y=cp.loc[:, "RouteRefSpeed"],
                                       line=dict(color='black', width=2, dash='dash'),
                                       name='reference speed'),
                            secondary_y=True, )

            if cp.loc[:, "RouteRefSpeed"].equals(cp.loc[:, "RouteRefSpeedUncapped"]):
                pass

            else:
                figcp.add_trace(go.Scatter(x=cp.loc[:, "Hour"],
                                           y=cp.loc[:, "RouteRefSpeedUncapped"],
                                           line=dict(color='black', width=2),
                                           name='RefSpd_Uncapped'),
                                secondary_y=True, )

            figcp.update_layout({
                'plot_bgcolor': "white",
                'paper_bgcolor': "white",
                'title': 'Route Percentile Speeds',
                'hovermode': 'x'
            })

            figcp.update_xaxes(title_text="Hour")

            figcp.update_yaxes(title_text="Speed(mph)", secondary_y=True, range=[5, 90])

            ####################################################

            # cardinal hourly delay figure
            figdc = go.Figure()
            figdc.add_trace(go.Scatter(x=cc.loc[cc.Year == 2018, "Hour"],
                                       y=cc.loc[cc.Year == 2018, "delay"],
                                       mode='lines+markers',
                                       marker=dict(color='red', ),
                                       line=dict(color='red', ),
                                       name='2018'))
            figdc.add_trace(go.Scatter(x=cc.loc[cc.Year == 2019, "Hour"],
                                       y=cc.loc[cc.Year == 2019, "delay"],
                                       mode='lines+markers',
                                       marker=dict(color='blue', ),
                                       line=dict(color='blue', ),
                                       name='2019'))
            ...
            figtdc = go.Figure(data=[
                go.Bar(name='cardinal', x=["2018", "2019"],
                       y= [cc.loc[cc.Year == 2018, "delay"].sum(),
                           cc.loc[cc.Year == 2019, "delay"].sum()
                           ])
            ])
            ...
            #Measures figure Cardinal
            #2018
            sh80_18 = mc18[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                      'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95_18 = mc18[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                      'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbi_18= mc18[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                      'BI_ltwe']]
            shlt_18 = [None, None, None, None]
            shlt_18.extend(mc18[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            x_values = ['Shift AM', 'Shift MD', 'Shift PM', 'Daytime', 'LOTTR AM', 'LOTTR MD','LOTTR PM', 'LOTTR WKEND']

            figmeasures18c = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80_18),
                go.Bar(name='PTI-95', x=x_values, y=sh95_18),
                go.Bar(name='BI', x=x_values, y=shbi_18),
                go.Bar(name='LOTTR', x=x_values, y=shlt_18)
            ])

            figmeasures18c.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Cardinal Travel TIme Reliability Measures 2018',
                'hovermode': 'x'
            })
            figmeasures18c.update_yaxes(range=[0, max(mc18[1:-1].max(),mcn18[1:-1].max()) + 0.5])

            #############################################################
            #without outliers
            sh80w_18 = mcw18[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                            'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95w_18 = mcw18[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                            'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbiw_18 = mcw18[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                            'BI_ltwe']]
            shltw_18 = [None, None, None, None]
            shltw_18.extend(mcw18[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            x_values = ['Shift AM', 'Shift MD', 'Shift PM', 'Daytime', 'LOTTR AM', 'LOTTR MD', 'LOTTR PM',
                        'LOTTR WKEND']

            figmeasuresw18c = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80w_18),
                go.Bar(name='PTI-95', x=x_values, y=sh95w_18),
                go.Bar(name='BI', x=x_values, y=shbiw_18),
                go.Bar(name='LOTTR', x=x_values, y=shltw_18)
            ])

            figmeasuresw18c.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Cardinal Travel TIme Reliability Measures 2018 without outliers',
                'hovermode': 'x'
            })
            figmeasuresw18c.update_yaxes(range=[0, max(mcw18[1:-1].max(), mcwn18[1:-1].max()) + 0.5])
            ...
            #2019
            sh80_19 = mc19[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                            'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95_19 = mc19[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                            'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbi_19 = mc19[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                            'BI_ltwe']]
            shlt_19 = [None, None, None, None]
            shlt_19.extend(mc19[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            x_values = ['Shift AM', 'Shift MD', 'Shift PM', 'Daytime', 'LOTTR AM', 'LOTTR MD', 'LOTTR PM',
                        'LOTTR WKEND']

            figmeasures19c = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80_19),
                go.Bar(name='PTI-95', x=x_values, y=sh95_19),
                go.Bar(name='BI', x=x_values, y=shbi_19),
                go.Bar(name='LOTTR', x=x_values, y=shlt_19)
            ])

            figmeasures19c.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Cardinal Travel Time Reliability Measures 2019',
                'hovermode': 'x'
            })

            figmeasures19c.update_yaxes(range=[0,max(mc19[1:-1].max(),mcn19[1:-1].max()) + 0.5])

            ########################################
            #without outliers
            sh80w_19 = mcw19[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                            'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95w_19 = mcw19[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                            'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbiw_19 = mcw19[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                            'BI_ltwe']]
            shltw_19 = [None, None, None, None]
            shltw_19.extend(mcw19[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            x_values = ['Shift AM', 'Shift MD', 'Shift PM', 'Daytime', 'LOTTR AM', 'LOTTR MD', 'LOTTR PM',
                        'LOTTR WKEND']

            figmeasuresw19c = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80w_19),
                go.Bar(name='PTI-95', x=x_values, y=sh95w_19),
                go.Bar(name='BI', x=x_values, y=shbiw_19),
                go.Bar(name='LOTTR', x=x_values, y=shltw_19)
            ])

            figmeasuresw19c.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Cardinal Travel Time Reliability Measures 2019 without outliers',
                'hovermode': 'x'
            })

            figmeasuresw19c.update_yaxes(range=[0, max(mcw19[1:-1].max(), mcwn19[1:-1].max()) + 0.5])
            ...
            fignc.add_trace(go.Bar(name='Volume',
                                   x=ncc.loc[ncc.Year == 2018, "Hour"],
                                   y=ncc.loc[ncc.Year == 2018, "hrvol"],
                                   marker_color=bar_color,
                                   opacity=0.5, ),
                            secondary_y=False, )

            fignc.add_trace(go.Scatter(x=ncc.loc[ncc.Year == 2018, "Hour"],
                                       y=ncc.loc[ncc.Year == 2018, "RouteMeanSpeed_dt"],
                                       mode='lines+markers',
                                       name='2018 Speed',
                                       marker=dict(color='red', ),
                                       line=dict(color='red')),
                            secondary_y=True, ),
            fignc.add_trace(go.Scatter(x=ncc.loc[ncc.Year == 2019, "Hour"],
                                       y=ncc.loc[ncc.Year == 2019, "RouteMeanSpeed_dt"],
                                       mode='lines+markers',
                                       name='2019 Speed',
                                       marker=dict(color='blue'),
                                       line=dict(color='blue')),
                            secondary_y=True, ),
            fignc.add_trace(go.Scatter(x=ncc.loc[ncc.Year == 2018, "Hour"],
                                       y=ncc.loc[ncc.Year == 2018, "RouteRefSpeed"],
                                       line=dict(color='green', width=2, dash='dash'),
                                       name='Reference Speed'),
                            secondary_y=True, ),
            fignc.update_layout({
            ...
            fignc.update_yaxes(title_text="Speed (mph)", range=[5, 90], secondary_y=True)
            fignc.update_yaxes(title_text="Volume", secondary_y=False)


###########################################################################################
            figncp = make_subplots(specs=[[{"secondary_y": True}]])

            figncp.add_trace(go.Scatter(x=ncp.loc[:, "Hour"],
                                        y=ncp.loc[:, 'RouteMeanSpeed_dt'],
                                        mode='lines+markers',
                                        name='Mean Spd',
                                        marker=dict(color='orange', ),
                                        line=dict(color='orange')),
            ...
            figncp.add_trace(go.Scatter(x=ncp.loc[:, "Hour"],
                                        y=ncp.loc[:, "RouteRefSpeed"],
                                        line=dict(color='black', width=2, dash='dash'),
                                        name='reference speed'),
                             secondary_y=True, )

            if ncp.loc[:, "RouteRefSpeed"].equals(ncp.loc[:, "RouteRefSpeedUncapped"]):
                pass

            else:
                figncp.add_trace(go.Scatter(x=ncp.loc[:, "Hour"],
                                            y=ncp.loc[:, "RouteRefSpeedUncapped"],
                                            line=dict(color='black', width=2),
                                            name='RefSpd_Uncapped'),
                                 secondary_y=True, )

            figncp.update_layout({
                'plot_bgcolor': "white",
                'paper_bgcolor': "white",
                'title': 'Route Percentile Speeds',
                'hovermode': 'x'
            ...
            figncp.update_xaxes(title_text="Hour")

            figncp.update_yaxes(title_text="Speed(mph)", secondary_y=True, range=[5, 90])
########################################################################################################

            # non-cardinal hourly delay figure
            figdnc = go.Figure()
            figdnc.add_trace(go.Scatter(x=ncc.loc[ncc.Year == 2018, "Hour"],
                                        y=ncc.loc[ncc.Year == 2018, "delay"],
                                        mode='lines+markers',
                                        marker=dict(color='red',),
                                        line=dict(color='red',),
                                        name='2018'))
            figdnc.add_trace(go.Scatter(x=ncc.loc[ncc.Year == 2019, "Hour"],
                                        y=ncc.loc[ncc.Year == 2019, "delay"],
                                        mode='lines+markers',
                                        marker=dict(color='blue', ),
                                        line=dict(color='blue', ),
                                        name='2019'))
            ...
            figtdnc = go.Figure(data=[
                go.Bar(name='noncardinal', x=['2018', '2019'],
                       y=[ncc.loc[ncc.Year == 2018, "delay"].sum(),
                          ncc.loc[ncc.Year == 2019, "delay"].sum()
                           ])
            ])
            ...
            #non-cardinal measrues figures
            #2018
            sh80n_18 = mcn18[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                          'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95n_18 = mcn18[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                          'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbin_18 = mcn18[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                          'BI_ltwe']]
            shltn_18 = [None, None, None, None]
            shltn_18.extend(mcn18[['LOTTR_AM','LOTTR_MD','LOTTR_PM','LOTTR_WE']].to_list())

            figmeasures18nc = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80n_18),
                go.Bar(name='PTI-95', x=x_values, y=sh95n_18),
                go.Bar(name='BI', x=x_values, y=shbin_18),
                go.Bar(name='LOTTR', x=x_values, y=shltn_18)
            ])

            figmeasures18nc.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Non-Cardinal Travel Time Reliability Measures 2018',
                'hovermode': 'x'
            })

            figmeasures18nc.update_yaxes(range=[0, max(mc18[1:-1].max(),mcn18[1:-1].max()) + 0.5])


            ##################################################
            #without outliers
            sh80wn_18 = mcwn18[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                              'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95wn_18 = mcwn18[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                              'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbiwn_18 = mcwn18[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                              'BI_ltwe']]
            shltwn_18 = [None, None, None, None]
            shltwn_18.extend(mcwn18[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            figmeasuresw18nc = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80wn_18),
                go.Bar(name='PTI-95', x=x_values, y=sh95wn_18),
                go.Bar(name='BI', x=x_values, y=shbiwn_18),
                go.Bar(name='LOTTR', x=x_values, y=shltwn_18)
            ])

            figmeasuresw18nc.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Non-Cardinal Travel Time Reliability Measures 2018 without outliers',
                'hovermode': 'x'
            })

            figmeasuresw18nc.update_yaxes(range=[0, max(mcw18[1:-1].max(), mcwn18[1:-1].max()) + 0.5])
            ################################################

            #2019
            sh80n_19 = mcn19[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                              'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95n_19 = mcn19[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                              'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbin_19 = mcn19[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                              'BI_ltwe']]
            shltn_19 = [None, None, None, None]
            shltn_19.extend(mcn19[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            figmeasures19nc = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80n_19),
                go.Bar(name='PTI-95', x=x_values, y=sh95n_19),
                go.Bar(name='BI', x=x_values, y=shbin_19),
                go.Bar(name='LOTTR', x=x_values, y=shltn_19)
            ])

            figmeasures19nc.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Non-Cardinal Travel Time Reliability Measures 2019',
                'hovermode': 'x'
            })

            figmeasures19nc.update_yaxes(range=[0,max(mc19[1:-1].max(),mcn19[1:-1].max()) + 0.5])

            ############################################################
            #without outliers
            sh80wn_19 = mcwn19[['PTI-80_sham', 'PTI-80_shmd', 'PTI-80_shpm', 'PTI-80_dt', 'PTI-80_ltam', 'PTI-80_ltmd',
                              'PTI-80_ltpm', 'PTI-80_ltwe']]
            sh95wn_19 = mcwn19[['PTI-95_sham', 'PTI-95_shmd', 'PTI-95_shpm', 'PTI-95_dt', 'PTI-95_ltam', 'PTI-95_ltmd',
                              'PTI-95_ltpm', 'PTI-95_ltwe']]
            shbiwn_19 = mcwn19[['BI_sham', 'BI_shmd', 'BI_shpm', 'BI_dt', 'BI_ltam', 'BI_ltmd', 'BI_ltpm',
                              'BI_ltwe']]
            shltwn_19 = [None, None, None, None]
            shltwn_19.extend(mcwn19[['LOTTR_AM', 'LOTTR_MD', 'LOTTR_PM', 'LOTTR_WE']].to_list())

            figmeasuresw19nc = go.Figure(data=[
                go.Bar(name='PTI-80', x=x_values, y=sh80wn_19),
                go.Bar(name='PTI-95', x=x_values, y=sh95wn_19),
                go.Bar(name='BI', x=x_values, y=shbiwn_19),
                go.Bar(name='LOTTR', x=x_values, y=shltwn_19)
            ])

            figmeasuresw19nc.update_layout({
                'plot_bgcolor': "#F9F9F9",
                'paper_bgcolor': "#F9F9F9",
                'title': 'Non-Cardinal Travel Time Reliability Measures 2019 without outliers',
                'hovermode': 'x'
            })

            figmeasuresw19nc.update_yaxes(range=[0, max(mcw19[1:-1].max(), mcwn19[1:-1].max()) + 0.5])
            #######################################################
            ...
            graphs_return1819 = html.Div([
                ...
                html.Div(className="row flex-display", children=[
                    html.Div([dcc.Graph(id="Percentile", figure=figcp)],
                             className="pretty_container six columns"),
                    html.Div([dcc.Graph(id="Percentile", figure=figncp)],
                             className="pretty_container six columns")
                ]),
                html.Div(className="row flex-display", children=[
                ...
                html.Div(className="row flex-display", children=[
                    html.Div([dcc.Graph(id="measures18c", figure=figmeasures18c)],
                             className="pretty_container six columns"),
                    html.Div([dcc.Graph(id="measures18nc", figure=figmeasures18nc)],
                             className="pretty_container six columns")
                ]),
                html.Div(className="row flex-display", children=[
                    html.Div([dcc.Graph(id="measures19c", figure=figmeasures19c)],
                             className="pretty_container six columns"),
                    html.Div([dcc.Graph(id="measures19nc", figure=figmeasures19nc)],
                             className="pretty_container six columns")
                ]),
                html.Div(className="row flex-display", children=[
                    html.Div([dcc.Graph(id="measuresw18c", figure=figmeasuresw18c)],
                             className="pretty_container six columns"),
                    html.Div([dcc.Graph(id="measuresw18nc", figure=figmeasuresw18nc)],
                             className="pretty_container six columns")
                ]),
                html.Div(className="row flex-display", children=[
                    html.Div([dcc.Graph(id="measuresw19c", figure=figmeasuresw19c)],
                             className="pretty_container six columns"),
                    html.Div([dcc.Graph(id="measuresw19nc", figure=figmeasuresw19nc)],
                             className="pretty_container six columns")
                ]),
            ]),
            ...
            ll = lendiction(df15['LIDDir'], df15['Length'])
