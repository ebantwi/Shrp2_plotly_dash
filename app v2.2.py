...
measures =  pd.read_csv('data/measures_18_19.csv')
...
######################################################################
            mclwa = lambda x: round(np.average(x, weights=mc.loc[x.index, 'Length']), 2)
########################################################################
            # CARDINAL
...
###################################################################
            mclwan = lambda x: round(np.average(x, weights=mcn.loc[x.index, 'Length']), 2)
###################################################################
...
#####################################################################
            ...
            if 1 in df['F_SYSTEM'].unique():
                ...
                try:
                    rfn = round(np.average(dn.loc[(dn.Hour >= 6) & (dn.Hour < 20), 'refspeed'],  #change refspeed to average speed.
